...
namespace TabML.Editor.Rendering
{
    class NoteRenderer
    {
        ...
        {
            var noteHeadOffset =
                drawingContext.ColumnRenderingInfos[beat.Column.ColumnIndex].GetNoteHeadOffset(note.String);
            var isHalfOrLonger = beat.NoteValue.Base >= BaseNoteValue.Half;
            if (note.EffectTechnique == NoteEffectTechnique.DeadNote)
            {
                drawingContext.DrawDeadNote(note.String, beat.Column.GetPosition(drawingContext),
                                            noteHeadOffset, isHalfOrLonger);
            }
            else if (note.Fret == BeatNote.UnspecifiedFret)
            {
                drawingContext.DrawPlayToChordMark(note.String, beat.Column.GetPosition(drawingContext),
                                                   noteHeadOffset, isHalfOrLonger);
            }
            else
            {
                drawingContext.DrawFretNumber(note.String, note.Fret.ToString(), beat.Column.GetPosition(drawingContext),
                                              noteHeadOffset, isHalfOrLonger);
            }
        ...
        private void DrawPostConnection(BarDrawingContext drawingContext, BeatNote note, BeamSlope beamSlope)
        {
            switch (note.PostConnection)
            {
                case PostNoteConnection.SlideOutToHigher:
                    drawingContext.DrawGliss(note.OwnerBeat.Column.GetPosition(drawingContext), note.String,
        ...
        private void DrawTie(BarDrawingContext drawingContext, BeatNote note, string instruction, BeamSlope beamSlope)
        {
            if (note.PreConnectedNote != null) // todo: handle cross-bar ties
            {
                ...
                drawingContext.DrawTie(note.PreConnectedNote.OwnerBeat.Column.GetPosition(drawingContext),
                                       note.OwnerBeat.Column.GetPosition(drawingContext), note.String,
        ...
        private double GetInstructionY(BarDrawingContext drawingContext, BeatNote note, BeamSlope beamSlope)
        {
            ...
            else
            {
                var instructionX = note.PreConnectedNote != null
                    ? (note.OwnerBeat.Column.GetPosition(drawingContext) +
                       note.PreConnectedNote.OwnerBeat.Column.GetPosition(drawingContext))/2
                    : note.OwnerBeat.Column.GetPosition(drawingContext);
